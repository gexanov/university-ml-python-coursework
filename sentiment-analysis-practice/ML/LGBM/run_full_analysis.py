...
def fit_final_model(X_train, y_train, best_params):
    ...
    joblib.dump(final_model, ARTIFACTS_DIR / "lgbm_final_model.joblib")
...
def save_metrics_and_curves(final_model, X_test, y_test, y_train):
...
def save_importances_and_pdp(final_model, X_train, X_val, y_val):
...
def main():
    print("Running full LGBM analysis")
    print(
        "CPU_THREADS =",
        CPU_THREADS,
        "USE_GPU =",
    ...
    save_metrics_and_curves(final_model, X_test, y_test, y_train)
    save_importances_and_pdp(final_model, X_train, X_val, y_val)
